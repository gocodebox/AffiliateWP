...
class Affiliate_WP_LifterLMS extends Affiliate_WP_Base {
	...
	*/
	public function init() {
		...
		// create pending referral and then mark it complete immediately after
		// b/c there's no "pending" order status in LLMS
		...
		// add link to llms order on referral screen
		add_filter( 'affwp_referral_reference_column', array( $this, 'reference_link' ), 10, 2 );

		// add affiliate product fields to courses & memberships
		...
		// save affiliate product fields to postmeta
		...
		// save affiliate coupon fields to postmeta
	 ...
	 *
	 * LifterLMS doesn't have pending order status but we might in the future
	 *
	...
	public function coupon_meta_save( $post_id, $post ) {
		...
		// we need either a username or userid to locate the affiliate with
		...
		// locate an affiliate, looks like this returns null if user doesn't have an affiliate acct
	 ...
	 * @return mixed              object of order related data or false
	 *
	 * @access private
	 */
	 ...
	 * @return mixed              affiliate id or false
	 *
	 * @access private
	 */
	...
	/**
	 * Retrive an array of product information to pass to affwp when creating a referral
	 *
	 * LifterLMS doesn't have the ability to purchase multiple products simultaneously
	 * but this is still returning an array of arrays just in case that matters somewhere
	 *
	...
	public function product_meta_save( $post_id, $post ) {
		...
		// if overrides is set, update the override related fields
